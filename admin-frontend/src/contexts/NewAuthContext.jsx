...
export const NewAuthProvider = ({ children }) => {
  ...
  const backendUrl = import.meta.env.VITE_ADMIN_BACKEND_URL || 'http://localhost:5174';
  ...
  useEffect(() => {
    const bootstrapSession = async () => {
      ...
      if (currentSession) {
        ...
        // Try restore demo session from localStorage (dev fallback from backend)
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, supaSession) => {
      const stored = getStoredDemo();
      // If we have an active demo session and Supabase reports null, don't clear our local demo session
  ...
  const signUp = async (email, password, hcaptchaToken) => {
    // Use Supabase client directly to avoid Bot Protection 400 errors
    try {
  ...
  const signIn = async (email, password, hcaptchaToken) => {
    // Use Supabase client directly to avoid HTTP 400 from manual REST calls
    try {
      const normalizedEmail = (email || '').trim().toLowerCase();
